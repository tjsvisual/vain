...
class SettingsController extends Controller
{
    ...
    public function actionAdsense($id = false)
    {
        $model = Adsense::find()->all();
        if($id)
        {
            $new = Adsense::findOne($id);
            $edit = true;
        }
        else
        {
        ...
        if($new->load(Yii::$app->request->post())) {
            $new->save(false);
            Yii::$app->getSession()->setFlash('success', 'Ad Create successfully');
